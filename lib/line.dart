...
class CustomLineChartWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final minY = temperatureComparisons
            .expand((comparison) => [
                  comparison.historicalTemperature,
                  comparison.currentTemperature
                ])
            .reduce(min)
            .floorToDouble() -
        5;
    ...
    return LineChart(
      LineChartData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (value, meta) {
                ...
                final interval =
                    calculateLabelInterval(temperatureComparisons.length);

                if (index < 0 || index >= temperatureComparisons.length) {
                  return const Text('');
                }
                if (index == temperatureComparisons.length - 1 ||
                    index % interval == 0) {
        ...
        lineBarsData: [
          LineChartBarData(
            spots: temperatureComparisons
                .asMap()
                .entries
                .map((e) =>
                    FlSpot(e.key.toDouble(), e.value.historicalTemperature))
                .toList(),
            isCurved: true,
            color: AppColors.red,
            barWidth: 2,
          ...
          LineChartBarData(
            spots: temperatureComparisons
                ...
                .entries
                .map(
                    (e) => FlSpot(e.key.toDouble(), e.value.currentTemperature))
                .toList(),
            isCurved: true,
